    ...
    : IRequestHandler<GetZoneStateCommand, CommandResult<ZoneState>>
{
    ...
    public GetZoneStateCommandHandler(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    )
    {
        ...
        {
            return new CommandResult<ZoneState>("Not Authorized");
        }
        var accessToken = accessTokenResult.Result;
        var zoneDetails = request.ZoneDetails;
        var zoneState = new ZoneStateModel { Zone = zoneDetails };
        ...
        var connectionStartResult = await _mediator.Send(
            new StartConnectionToZoneServerCommand(accessToken, zoneDetails),
            cancellationToken
        );
        ...
        {
            return new CommandResult<ZoneState>(editorResult.ErrorCode);
        }
        ...
        {
            return new CommandResult<ZoneState>(zoneEditorState.ErrorCode);
        }
        ...
        {
            return new CommandResult<ZoneState>(scriptErrorDetails.ErrorCode);
        }
        ...
        return new CommandResult<ZoneState>(zoneState);
    }
}
