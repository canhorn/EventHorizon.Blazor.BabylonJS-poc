...
public class AssetFileWindowModel : EditorComponentBase
{
    ...
    public async Task HandleDirectoryContentClicked(FileSystemDirectoryContent directoryContent)
    {
        await Mediator.Publish(new AssetFileDirectoryContentClickedEvent(directoryContent));
    }

    public async Task HandleDeleteFileDirectoryContent(FileSystemDirectoryContent directoryContent)
    {
        await Mediator.Publish(new AssetFileDeleteTriggeredEvent(directoryContent));
    }
}
