namespace EventHorizon.Game.Editor.Client.AssetManagement.Components.Breadcrumb
{
    ...
    public class AssetManagementBreadcrumbModel
        : EditorComponentBase
    {
        [CascadingParameter]
        ...
        public List<BreadcrumbItem> BreadcrumbItemList { get; private set; } = new List<BreadcrumbItem>();
        ...
        protected async Task HandleNavigateToPath(
            string filterPath
        )
        {
            await Mediator.Send(
                new LoadAssetServerFilterPathCommand(
                    filterPath
                )
            );
        }
        ...
        private void Setup()
        {
            var path = State.CurrentWorkingDirectory.FilterPath.Split(
            ...
            if (currentLocation.IsNotNullOrEmpty()
