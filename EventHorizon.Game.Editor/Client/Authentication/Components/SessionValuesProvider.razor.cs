        ...
        SessionValueSetEventObserver
{
    ...
    {
        var result = await Mediator.Send(new FillSessionValuesCommand());
        if (result)
        {
    ...
    public async Task Handle(SessionValueSetEvent args)
    {
        var result = await Mediator.Send(new QueryForSessionValues());
        if (result)
        {
            SessionValues = result.Result;
        }
