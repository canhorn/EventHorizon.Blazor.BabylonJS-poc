...
public class ProcessGraph : Graph<ProcessStep, NodePortReference>
{
    ...
    public bool Connect(ProcessStep start, NodePort outputPort, ProcessStep end, NodePort inputPort)
    {
        return Connect(
            ...
            new NodePortReference { FromPortName = outputPort.Name, ToPortName = inputPort.Name, }
        );
    }
    ...
    public override bool Connect(ProcessStep start, ProcessStep end, NodePortReference data)
    {
        ...
        if (outputPort == null)
        {
            ...
            throw new ArgumentException(
                $"Start node doesn't have an output port named '{data.FromPortName}'"
            );
        }
        ...
        if (inputPort == null)
        {
            // Verify port exists
            throw new ArgumentException(
                $"End node doesn't have an input port named '{data.ToPortName}'"
            );
        }
        if (!inputPort.CanStore(outputPort.GetStorageType()))
        {
            ...
            throw new ArgumentException(
                $"Cannot connect port output of type {outputPort.GetStorageType()} to one accepting {inputPort.GetStorageType()}"
            );
        }

        // Clean up extra connections to the same port
        this.DisconnectAll(
            (linkStart, linkEnd, mapping) => linkEnd == end && mapping.ToPortName == data.ToPortName
        );
    ...
    private void Recompute(ProcessStep node)
    {
        ...
        foreach (var input in inputs)
        {
            foreach (var edge in GetAllEdgeData(input, node))
            {
                if (
                    edge == null
                    || string.IsNullOrEmpty(edge.FromPortName)
                    || string.IsNullOrEmpty(edge.ToPortName)
                )
                {
                    continue;
                }
                ...
                if (inputPort != null && outputPort != null && outputPort.HasValue)
                {
