...
{
    public static IServiceCollection AddErrorBoundary(
        this IServiceCollection services
    )
    {
        var errorBoundaryService = new StandardErrorBoundaryService();
        var platformConsoleLoggerProvider =
            new StandardErrorBoundaryLoggerProvider(errorBoundaryService);

        services.AddSingleton<ErrorBoundaryService>(errorBoundaryService);

        services.AddLogging(
            config => config.AddProvider(platformConsoleLoggerProvider)
        );
