    ...
    : IRequestHandler<SetupGuiLayoutCommand, StandardCommandResult>
{
    ...
    {
        var control = _controlState.Get(guiId, layoutControl.Id);
        if (!control.HasValue)
        {
            return;
        }
        ...
        if (parent.HasValue)
        {
            parent.Value.AddControl(control.Value);
        }
        else
        {
            _renderingGui.GetGuiCanvas().AddControl(control.Value);
        }
        if (layoutControl.LinkWith != null)
        {
            control.Value.LinkWith(layoutControl.LinkWith);
        }

        foreach (
            var layoutChildControl in (
                layoutControl.ControlList ?? new List<IGuiLayoutControlData>()
            ).OrderBy(a => a.Sort)
        )
        {
            AddControlToLayout(guiId, layoutChildControl, control);
        }
    }
}
