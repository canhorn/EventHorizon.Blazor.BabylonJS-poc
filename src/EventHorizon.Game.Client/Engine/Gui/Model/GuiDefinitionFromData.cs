...
public class GuiDefinitionFromData : ClientLifecycleEntityBase, IGuiDefinition
{
    ...
    public GuiDefinitionFromData(
        ...
        : base(new ObjectEntityDetailsModel())
    {
        GuiId = id;
        ...
        _scriptData = new ScriptData(new Dictionary<string, object>());
    }

    public override async Task Initialize()
    {
        var initializeScript = await GetClientScript(_layout.InitializeScript);
        if (initializeScript.IsNotNull())
        {
            await initializeScript.Run(_scriptServices, _scriptData);
        }
    ...
    {
        var disposeScript = await GetClientScript(_layout.DisposeScript);
        if (disposeScript.IsNotNull())
        {
            await disposeScript.Run(_scriptServices, _scriptData);
        }
    ...
    public async Task Activate()
    {
        ...
        {
            await activateScript.Run(_scriptServices, _scriptData);
        }
    }
    ...
    public async Task LinkWith(object linkWith)
    {
        foreach (var control in _layout.ControlList)
        {
            await _mediator.Send(
    ...
    )
    {
        ...
        var text = OptionTextFromKey(controlOptions);
        ...
        {
            if (controlOptions.TryGetValue(modelOption, out var controlOption))
            {
    ...
    {
        var onClick = options.GetValue<Func<Task>>("onClick");
        if (onClick.HasValue)
        {
    ...
    {
        return _controlDataList.FirstOrDefault(a => a.ControlId == controlId);
    }
