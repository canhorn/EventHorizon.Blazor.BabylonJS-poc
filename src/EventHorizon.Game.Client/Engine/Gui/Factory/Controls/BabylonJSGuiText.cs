...
public class BabylonJSGuiText : IBabylonJSGuiControl
{
    ...
    private TextBlock CreateControl(string id, IGuiControlOptions options)
    {
        var textControl = new TextBlock($"{id}-text");
        textControl.isHitTestVisible = false;

        Update(options, textControl);
    ...
    private void Update(IGuiControlOptions options, TextBlock textControl)
    {
        foreach (var option in options)
        {
            if (!IGNORE_PROPERTY_LIST.Contains(option.Key))
            {
                SetPropertyOnControl(textControl, option.Key, option.Value);
            }
        }
    }
}
