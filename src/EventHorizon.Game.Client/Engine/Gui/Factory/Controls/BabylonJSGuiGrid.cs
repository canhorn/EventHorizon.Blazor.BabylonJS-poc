...
public class BabylonJSGuiGrid : IBabylonJSGuiControl
{
    ...
    public BabylonJSGuiGrid(string id, IGuiControlOptions options, IGuiGridLocation? gridLocation)
    {
        Id = id;
        Options = options;
    ...
    private Grid CreateControl(string id, IGuiControlOptions options)
    {
        var gridControl = new Grid($"{id}_grid");
        gridControl.isHitTestVisible = false;

        Update(options, gridControl);
    ...
    private void Update(IGuiControlOptions options, Grid gridControl)
    {
        foreach (var option in options)
        {
            if (!IGNORE_PROPERTY_LIST.Contains(option.Key))
            {
                SetPropertyOnControl(gridControl, option.Key, option.Value);
            }
        }
    }
