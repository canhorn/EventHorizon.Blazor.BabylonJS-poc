...
public class StandardI18nState : II18nState, ILocalizer
{
    ...
    public string this[string name]
    {
        get
        {
            if (_bundle.TryGetValue(name, out var value))
            {
                return value;
            }
            return name;
    ...
    {
        get { return string.Format(this[name], arguments); }
    }

    public string Template(string template, IDictionary<string, object> data)
    {
    ...
    public void SetResourceBundle(II18nBundle bundle)
    {
        var newBundleDictionary = new List<II18nBundle> { _bundle, bundle }
            .SelectMany(a => a)
            .ToLookup(pair => pair.Key, pair => pair.Value)
            .ToDictionary(a => a.Key, a => a.Last());
        _bundle = new I18nBundleModel(newBundleDictionary);
    }
}
