namespace EventHorizon.Platform.LogProvider.Logger
{
    ...
    public class PlatformConsoleLogger
        : ILogger
    {
        private readonly static JsonSerializerOptions JSON_OPTIONS = new JsonSerializerOptions { WriteIndented = true };
        ...
        )
        {
            ...
            _logEnrichmentService.EnrichReference(
                stateAsDictionary
            );
            ...
            {
                Console.WriteLine($"[{timestamp:O}] [{logLevel}] [{_name}[{eventId.Id}]] {formatter(state, exception!)}");
