...
public class Node : CachedEntityObject, IBehaviorAware<Node>
{
    ...
    public static ActionCallback Construct(
        string type,
        string name,
        Scene scene,
        object options = null
    )
    {
        return EventHorizonBlazorInterop.Func<ActionCallback>(
            new object[]
            {
                new string[] { "BABYLON", "Node", "Construct" },
                type,
                name,
                scene,
                options
    ...
    public AbstractMesh[] getChildMeshes(
        System.Nullable<bool> directDescendantsOnly = null,
        ActionCallback<Node> predicate = null
    )
    {
    ...
    public string getChildren(Func<Node, Task> callback)
    {
        ...
        _getChildrenActionMap.Add(handle, callback);

        return handle;
    }

    public bool getChildren_Remove(string handle)
    {
        return _getChildrenActionMap.Remove(handle);
    }
