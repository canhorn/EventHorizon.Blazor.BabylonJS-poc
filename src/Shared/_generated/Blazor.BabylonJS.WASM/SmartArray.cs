    ...
    where T : CachedEntity, new()
{
    ...
    private readonly IDictionary<string, Func<T, Task>> _forEachActionMap =
        new Dictionary<string, Func<T, Task>>();

    public string forEach(Func<T, Task> callback)
    {
        SetupForEachLoop();

        var handle = Guid.NewGuid().ToString();
        _forEachActionMap.Add(handle, callback);

        return handle;
    }

    public bool forEach_Remove(string handle)
    {
        return _forEachActionMap.Remove(handle);
    }
    ...
    private readonly IDictionary<string, Func<T, T, Task>> _sortActionMap =
        new Dictionary<string, Func<T, T, Task>>();

    public string sort(Func<T, T, Task> callback)
    {
        SetupSortLoop();

        var handle = Guid.NewGuid().ToString();
        _sortActionMap.Add(handle, callback);

        return handle;
    }

    public bool sort_Remove(string handle)
    {
        return _sortActionMap.Remove(handle);
    }
