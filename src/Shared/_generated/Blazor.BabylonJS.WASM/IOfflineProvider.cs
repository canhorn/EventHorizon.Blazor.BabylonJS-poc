...
public class IOfflineProviderCachedEntity : CachedEntityObject, IOfflineProvider
{
    ...
    private readonly IDictionary<string, Func<Task>> _openActionMap =
        new Dictionary<string, Func<Task>>();

    public string open(Func<Task> callback)
    {
        SetupOpenLoop();

        var handle = Guid.NewGuid().ToString();
        _openActionMap.Add(handle, callback);

        return handle;
    }

    public bool open_Remove(string handle)
    {
        return _openActionMap.Remove(handle);
    }
