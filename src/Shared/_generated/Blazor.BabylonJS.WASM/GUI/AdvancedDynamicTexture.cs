...
public class AdvancedDynamicTexture : DynamicTexture
{
    ...
    public Control[] getDescendants(
        ...
        ActionCallback<Control> predicate = null
    )
    {
    ...
    public string executeOnAllControls(Func<Control, Task> callback)
    {
        ...
        _executeOnAllControlsActionMap.Add(handle, callback);

        return handle;
    }

    public bool executeOnAllControls_Remove(string handle)
    {
        return _executeOnAllControlsActionMap.Remove(handle);
    }
