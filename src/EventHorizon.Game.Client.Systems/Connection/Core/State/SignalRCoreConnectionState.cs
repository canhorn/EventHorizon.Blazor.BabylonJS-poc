...
public class SignalRCoreConnectionState : CoreConnectionState
{
    ...
    public async Task StartConnection(string serverUrl, string accessToken)
    {
        if (_connection != null)
        {
            return;
    ...
    private async Task HandleAccountConnected(AccountInfoModel accountInfo)
    {
    ...
    private Task HandleConnectionClosed(Exception? ex)
    {
        ...
        return _mediator.Publish(new AccountDisconnectedEvent(code, ex));
    }
