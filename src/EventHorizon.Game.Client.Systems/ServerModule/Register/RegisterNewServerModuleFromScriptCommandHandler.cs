    ...
    >
{
    ...
    )
    {
        ...
        _scriptsState.Set(serverModuleScripts);

        var serverModuleResult = await _mediator.Send(
            new CreateEntityLifeCycleModuleCommand(serverModuleScripts),
            CancellationToken.None
        );
        if (!serverModuleResult.Success)
        {
            return new StandardCommandResult("failed_to_create_server_module");
        }
        var serverModule = serverModuleResult.Result;
        // Set and get any existing
        var current = _state.Set(serverModule);
        if (current.HasValue)
        {
            // If existing, dispose of ServerModule
            await _mediator.Send(
                new DisposeOfServerModuleCommand(current.Value.Name),
                cancellationToken
            );
        ...
        await _registerInitializable.Register(serverModule);
        await _registerDisposable.Register(serverModule);
        if (serverModule.IsUpdatable)
        {
            await _registerUpdatable.Register(serverModule);
        }
