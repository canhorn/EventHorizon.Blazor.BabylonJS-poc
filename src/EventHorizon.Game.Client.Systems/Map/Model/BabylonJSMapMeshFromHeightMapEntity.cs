        ...
        PointerHitMeshEventObserver
{
    ...
    public BabylonJSMapMeshFromHeightMapEntity(IMapMeshDetails details)
        : base(
                        ...
                        if (_mesh.IsNotNull())
                        {
                            await _mediator.Send(
                                new SetHeightResolverCoordinatesCommand(
                                    new BabylonJSHeightCoordinates(_mesh)
                                )
                            );
                        }
                        await _mediator.Publish(new MapMeshReadyEvent());
                    }
                ),
            },
            scene
    ...
    public override async Task PostInitialize()
    {
        ...
        if (_material.IsNotNull() && light.IsNotNull())
        {
    ...
    private async Task<Light?> GetLight()
    {
        var entityQueryList = await _mediator.Send(
            new QueryForEntity(TagBuilder.CreateNameTag(_mapDetails.Light))
        );
    ...
    public async Task Handle(PointerHitMeshEvent args)
    {
        if (_mesh.IsNull() || args.MeshName != _mesh.name)
        {
            return;
        }
        await _mediator.Publish(new MapMeshHitEvent(args.Position));
    }
}
