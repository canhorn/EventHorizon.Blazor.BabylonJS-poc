...
public class MoveModule : ModuleEntityBase, IMoveModule, ClientActionEntityMoveEventObserver
{
    ...
    public MoveModule(IObjectEntity entity)
        : base()
    {
    ...
    public void SetCurrentMoveTo(IVector3 position)
    {
        _currentMoveTo = position;
    }

    public async Task Handle(ClientActionEntityMoveEvent args)
    {
        if (args.EntityId != _entity.EntityId || args.MoveTo == null)
        {
            return;
        }
        await SetMoveTo(args.MoveTo);
    }

    private async Task SetMoveTo(IVector3 moveTo)
    {
        moveTo = MapMoveTo(moveTo);
    ...
    private IVector3 MapMoveTo(IVector3 moveTo)
    {
        return new StandardVector3(
            moveTo.X,
            _heightResolver.FindHeight(moveTo.X, moveTo.Z),
            moveTo.Z
        );
    }
}
