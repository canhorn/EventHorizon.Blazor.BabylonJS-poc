...
public class StandardInteractionModule : ModuleEntityBase, InteractionModule
{
    ...
    public StandardInteractionModule(IObjectEntity entity)
    {
        _entity = entity;
    }
    ...
    public override Task Initialize()
    {
        ...
        _interactionDistanceToPlayer =
            _interactionState.List?.Select(a => a.DistanceToPlayer).Max()
            ?? _interactionState.DistanceToPlayer;
    ...
    {
        var playerResult = await _mediator.Send(new QueryForCurrentPlayer());
        if (
