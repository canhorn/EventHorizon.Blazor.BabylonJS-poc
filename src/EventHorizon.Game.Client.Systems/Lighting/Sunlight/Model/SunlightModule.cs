namespace EventHorizon.Game.Client.Systems.Lighting.Sunlight.Model
{
        ...
        ISunlightModule
    {
        ...
        {
            if (_lightMovementVectorList.TryDequeue(
                out var lightMovementVector
            ))
            {
            ...
            else
            {
                ...
                {
                    _lightMovementVectorList.Enqueue(
                        lightVector
                    );
                }
            }
        ...
        public static class Curve3
        {
             ...
             */
            public static IEnumerable<IVector3> CreateCatmullRomSpline(IVector3[] points, int nbPoints, bool closed = false)
            {
                ...
                if (closed)
                {
                    ...
                    for (var i = 0; i < pointsCount; i++)
                    {
                        ...
                        {
                            catmullRom.Add(Vector3Math.CatmullRom(points[i % pointsCount], points[(i + 1) % pointsCount], points[(i + 2) % pointsCount], points[(i + 3) % pointsCount], amount));
                            amount += step;
                        }
                    }
                ...
                {
                    var totalPoints = new List<IVector3>();
                    totalPoints.Add(points[0].Clone());
                    totalPoints.AddRange(points);
                    totalPoints.Add(points[points.Length - 1].Clone());
                    var i = 0;
                    for (i = 0; i < totalPoints.Count - 3; i++)
                    {
                        ...
                        for (var c = 0; c < nbPoints; c++)
                        {
                            catmullRom.Add(Vector3Math.CatmullRom(totalPoints[i], totalPoints[i + 1], totalPoints[i + 2], totalPoints[i + 3], amount));
                            amount += step;
                        }
                    }
                    i--;
