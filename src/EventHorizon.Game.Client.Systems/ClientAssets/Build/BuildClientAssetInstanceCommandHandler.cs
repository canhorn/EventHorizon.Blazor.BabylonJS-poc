    ...
    : IRequestHandler<BuildClientAssetInstanceCommand, StandardCommandResult>
{
    ...
    )
    {
        ...
        // Check Mesh Cache
        else if (_cache.Cached(clientAsset.Id))
        {
            var cachedClientAsset = _cache.Get(clientAsset.Id);
        ...
        var loaderOption = _loaderState.Get(clientAsset.Type);
        ...
        if (loaderOption.HasValue)
        {
            ...
            await loader.Load(details, clientAsset);
            return new();
        }
