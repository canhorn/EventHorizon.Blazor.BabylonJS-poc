namespace EventHorizon.Game.Client.Systems.Dialog.Open
{
        ...
        : IRequestHandler<OpenDialogTreeCommand, StandardCommandResult>
    {
        ...
        private async ValueTask<(bool success, Option<DialogTree> dialogTree, Option<IPlayerEntity> player, Option<IObjectEntity> npc)> ValidateParameters(
            long playerId,
            long npcId,
            string dialogTreeId,
            CancellationToken cancellationToken
        )
        {
            ...
            if (!playerResult.Success
                || playerResult.Result.EntityId != playerId)
            {
                _logger.LogWarning(
