    ...
    : IRequestHandler<QueryForZoneDetails, CommandResult<IZoneDetails>>
{
    ...
    public QueryForZoneDetailsHandler(IAccountState state)
    {
        _state = state;
    }
    ...
    )
    {
        ...
        if (zoneDetails == null)
        {
            return new CommandResult<IZoneDetails>("zone_details_not_found").FromResult();
        }
        return new CommandResult<IZoneDetails>(zoneDetails).FromResult();
    }
}
