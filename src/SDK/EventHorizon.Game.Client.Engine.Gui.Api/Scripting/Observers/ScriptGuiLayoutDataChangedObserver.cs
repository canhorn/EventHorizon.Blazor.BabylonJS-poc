    ...
    : GuiLayoutDataChangedEventObserver
{
    ...
    /// <returns></returns>
    public static string DataKey(string layoutId, string guiId, string key) =>
        $"{layoutId}-{guiId}-{key}";
    ...
    )
    {
        ...
        _data.Set(DataKey(_layoutId, _guiId, "active"), false);
    }

    public Task Handle(GuiLayoutDataChangedEvent args)
    {
    ...
    public async Task OnChange()
    {
        ...
        {
            await _services.Mediator.Send(new DisposeOfGuiCommand(_guiId));
            _data.Set(DataKey(_layoutId, _guiId, "active"), false);
        }
        var result = await _services.Mediator.Send(
            new CreateGuiCommand(_guiId, _layoutId, _getGuiControlData())
        );

        if (result.Success)
        {
            await _services.Mediator.Send(new ActivateGuiCommand(_guiId));
            _data.Set(DataKey(_layoutId, _guiId, "active"), true);
        }
    }
}
